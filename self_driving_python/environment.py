...
class TrafficLight(object):
    # True = North-South is open, False = Eeat-West is open
    ...
    def update(self, t):
        if t - self.last_updated >= self.period:
            # assuming state is boolean
            self.state = not self.state  
            self.last_updated = t
            
# Environment within which all agents operate
class Environment(object):
    # None means stop at that stage
    ...
    # even if enforce_deadline is False, end trial when deadline reaches this value
    hard_time_limit = -100  
    ...
    def __init__(self, num_dummies=3):
        ...
        self.block_size = 100
        # OrderedDict() let all the values in different elements under order
