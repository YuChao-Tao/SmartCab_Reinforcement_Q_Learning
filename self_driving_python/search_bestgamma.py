...
class LearningAgent(Agent):
    ...
    def record_trial(self, action, reward, inputs, t, deadline):
        ...
        result['prob_func'] = detect_function(self.env.epsilon)
        return result
    ...
    def update(self, t):
        ...
        prob = cross_entropy(t)
        action = (np.random.choice([best_choice(self.q_table[self.state]),
                                    self.random_action()], 1, p=[1-prob, prob])[0])
...
if __name__ == '__main__':
    ...
    path = os.path.join(script_dir, '../results/search_bestgamma.json')

    # Gamma parameter search
    discount_factor = np.linspace(0.1, 1.1, 10, endpoint=False)
    for sim in range(1000):
        for g in discount_factor:
            run(alpha=decay4, gamma=g, epsilon=cross_entropy, initial_value='zero', sim_num=sim)
